...
TcpServer.prototype.getClient = function() {
...
function getTcpListener(self, server) {
  return function(conn) {
    ...
    // Create a client for the new connection
    server.client = require('../client').tcp(utils.merge(self.options, { socket: conn }))

    // Parse incoming messages until no more
    utils.parseStream(conn, options, function(err, request) {
      if(err) {
        return respondError(err);
      }

      if ( utils.Request.isValidRequest(request, options.version) || utils.Request.isBatch(request) ) {
      ...
      // Else this must be a response to a client request
      else {
        server.client._onIncomingMessage(conn, err, request);
      }
    });
  };
}
