...
class WebSocketServer extends WebSocket.Server {
  constructor(server, options) {
    ...
    this.on('connection', function(wsock/*, req*/) {
      self.server.client = require('../client').ws(utils.merge(self.options, { wsock: wsock }))
  ...
  onIncomingMessage(wsock, data) {
    ...
    utils.JSON.parse(data, options, function(err, msg) {
      ...
      // Else this may be a response to a client's request
      else {
        self.server.client._onIncomingMessage(wsock, msg);
      }
    });
  }
